...
function Navbar() {
  ...
  return (
    <div className={`${theme?"bg-[#16141b]":"bg-stone-100"} basis-1/6 border border-transparent ${theme?"border-r-stone-700":"border-r-stone-300"} overflow-hidden flex flex-col items-center`}>
      <img className="mb-3.5" src={logo} alt="logo"/>
      <div className="w-10 min-w-[2.5rem] h-10 min-h-[2.5rem] rounded-full bg-red-300"/>
      <div className={`border w-[70%] border-transparent ${theme?"border-b-stone-700":"border-b-stone-300"} flex flex-col text-xl mt-10 text-stone-600`}>
      ...
      <div onClick={
        ...
        <div className={`flex rounded-full p-1 justify-center items-center ${theme?"bg-[#16141b]":""}`}><FontAwesomeIcon className={`${theme?"text-sky-500":"text-white"} text-sm`} icon={faMoon}/></div>
      </div>
    </div>  
  )
}
